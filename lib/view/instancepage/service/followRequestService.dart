...
class FollowRequestService {
  ...
  static Future<FollowRequestModel?> followRequestService(
    id,
  ) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    // String baseUrl = (prefs.getString('url') ?? Urls.baseUrl);
    String? token = prefs.getString(Constants.authToken);
    final response = await client.post(Uri.parse(Urls.followrequest),
        body: json.encode({
          "requestSendTo": id,
        }),
        headers: {
    ...
    if (response.statusCode == 200) {
      // print(responseData);
      return followRequestModelFromJson(response.body);
