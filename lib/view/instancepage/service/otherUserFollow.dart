...
class OtherUserFollowService {
  static var client = http.Client();

  static Future<OtherUserFollow?> otherUserFollowService(
    id,
  ) async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    // String baseUrl = (prefs.getString('url') ?? Urls.baseUrl);
    String? token = prefs.getString(Constants.authToken);
    final response = await client.post(Uri.parse(Urls.otheruserfollow),
        body: json.encode({
          "userid": id,
        }),
        headers: {
    ...
    if (response.statusCode == 200) {
      // print(responseData);
      return otherUserFollowFromJson(response.body);
